    ...
    with p.oneshot():
        ...
        try:
            ...
            info["memory"] = {
                ...
                "shared_mb": round(getattr(mi, "shared", 0) / 1024 / 1024, 1),
    ...
    def parameters(self) -> dict:
        return {
            ...
            "properties": {
                ...
                "status_filter": {
                    "type": "string",
                    "description": "Filter by status: 'running', 'sleeping', 'zombie', etc. Omit for all."
                }
            },
            "required": []
        }

    def run(self, sort_by: str = "cpu", limit: int = 20, status_filter: str = None) -> dict:
        limit = min(limit, 50)
        ...
        for p in psutil.process_iter(attrs):
            try:
                ...
                procs.append({
                    ...
                    "cpu_percent": i.get("cpu_percent") or 0,
                    "memory_percent": round(i.get("memory_percent") or 0, 2),
                    "rss_mb": round(mi.rss / 1024 / 1024, 1) if mi else None,
                    "vms_mb": round(mi.vms / 1024 / 1024, 1) if mi else None,
...
class SearchProcess(BaseTool):
    ...
    def run(self, name: str = None, pid: int = None, include_env: bool = False) -> dict:
...
class KillProcess(BaseTool):
    ...
    description = (
        ...
        "Returns the process name and confirmation."
    )
    ...
    def run(self, pid: int) -> dict:
        if pid < PROTECTED_PID_THRESHOLD:
            return {
                "error": f"Refused: PID {pid} is below the protected threshold ({PROTECTED_PID_THRESHOLD}). "
        ...
        try:
            ...
            return {"error": f"Access denied: insufficient permissions to terminate PID {pid}."}
...
class SetProcessPriority(BaseTool):
    ...
    def run(self, pid: int, nice: int) -> dict:
        ...
        try:
            ...
            return {"error": f"Access denied: raising priority (lower nice) requires root/sudo."}