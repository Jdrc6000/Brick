    ...
    def run(self, host: str, count: int = 5, traceroute: bool = False) -> dict:
        ...
        try:
            ...
            for line in output.splitlines():
                m = re.search(r"(\d+\.?\d*)/(\d+\.?\d*)/(\d+\.?\d*)/(\d+\.?\d*)\s*ms", line)
                if m:
                    result["latency_ms"] = {
                        "min": float(m.group(1)),
                        "avg": float(m.group(2)),
                ...
                if m2:
                    result["latency_ms"] = {"avg": float(m2.group(1))}
                m3 = re.search(r"(\d+)%\s*(packet\s*)?loss", line, re.IGNORECASE)
                if m3:
                    result["packet_loss_percent"] = int(m3.group(1))
        ...
        if traceroute:
            try:
                tr_cmd = (
                    ["tracert", "-d", "-h", "20", host] if system == "Windows"
...
class GetNetworkIO(BaseTool):
    ...
    def run(self, interface: str = None, sample_seconds: float = 1.0) -> dict:
        def to_mb(b): return round(b / 1024 / 1024, 2)
        ...
        for iface, a in after.items():
            ...
            error_rate = round((a.errin + a.errout) / max(total_packets, 1) * 100, 3)
            drop_rate = round((a.dropin + a.dropout) / max(total_packets, 1) * 100, 3)
            ...
            result[iface] = {
                ...
                "rates": {
                    "send_mb_per_s": round((a.bytes_sent - b.bytes_sent) / sample_seconds / 1024 / 1024, 3),
                    "recv_mb_per_s": round((a.bytes_recv - b.bytes_recv) / sample_seconds / 1024 / 1024, 3),
                },
                "errors": {
                    "in": a.errin,
                    "out": a.errout,
