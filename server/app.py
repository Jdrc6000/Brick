...
def get_device(request: Request) -> dict:
    ...
    if not device:
        raise HTTPException(status_code=403, detail=f"Unknown device: {ip}")
    return device
...
class ChatRequest(BaseModel):
    message: str
    # device_name not needed — server always uses the caller's own identity


@app.post("/chat")
async def chat(req: ChatRequest, request: Request):
    # Every device chats as themselves, period.
    device = get_device(request)
